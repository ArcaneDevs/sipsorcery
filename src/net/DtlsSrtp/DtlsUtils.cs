...
// a public key on the P-256 curve and a signature with a SHA-256 hash."
//
...
namespace SIPSorcery.Net
{
    public class DtlsUtils
    {
        ...
        {
            PemObject pem = LoadPemResource(resource);
            if (pem.Type.EndsWith("RSA public KEY"))
            {
            ...
            if (pem.Type.EndsWith("public KEY"))
            {
                return PrivateKeyFactory.CreateKey(pem.Content);
            }
            throw new Exception("'resource' doesn't specify a valid public key");
        }
        ...
        public static X509Certificate2 CreateSelfSignedCert(string subjectName, string issuerName, AsymmetricKeyParameter privateKey)
        {
            ...
            // certificate and public key to a .NET certificate. The problem is that setting the public key on a .NET
            ...
            try
            {
                ...
                if (seq.Count != 9)
                {
                    throw new Org.BouncyCastle.OpenSsl.PemException("malformed sequence in RSA public key");
                }
        ...
        public static RSA CreateRSAProvider(RSAParameters rp)
        {
